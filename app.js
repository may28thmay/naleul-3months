...
(function () {
  ...
  /* 인라인 핸들러의 JS 문자열 인자용 — 백슬래시·따옴표를 이스케이프한 뒤 속성 컨텍스트로 */
  ...
  /* 가져온 백업을 스텝 타입별 스키마로 정규화 — 오염된 값이 렌더를 깨뜨리지 않게 */
  function strv(x) { return typeof x === "string" ? x : ""; }
  function strArr(x, n) {
    var a = [], len = Array.isArray(x) ? x.length : 0;
    for (var i = 0; i < len; i++) a.push(strv(x[i]));
    if (n != null) { a = a.slice(0, n); while (a.length < n) a.push(""); }
    return a;
  }
  function cleanVal(s, v) {
    var o, rows, plans, i;
    var isObj = v && typeof v === "object" && !Array.isArray(v);
    switch (s.type) {
      case "text": case "manifest":
        return typeof v === "string" ? v : undefined;
      case "choices":
        if (!isObj) return undefined;
        return { picked: strArr(v.picked).filter(function (x) { return x; }), other: strv(v.other) };
      case "mandala":
        if (!isObj) return undefined;
        return { center: strv(v.center), cells: strArr(v.cells, 8) };
      case "sliders":
        if (!isObj) return undefined;
        o = {};
        Object.keys(v).forEach(function (k) { var n = Number(v[k]); if (isFinite(n)) o[k] = n; });
        return o;
      case "dailyLog":
        rows = Array.isArray(v) ? v.slice(0, 7) : [];
        while (rows.length < 7) rows.push({});
        return rows.map(function (r) {
          var row = {};
          s.fields.forEach(function (f) { row[f.key] = strv(r && typeof r === "object" ? r[f.key] : ""); });
          return row;
        });
      case "reframe":
        if (!isObj) return undefined;
        return { resonate: strv(v.resonate), doubt: strv(v.doubt), mine: strv(v.mine) };
      case "journey":
        if (!isObj) return undefined;
        return { v: strArr(v.v, 5), where: strv(v.where), surprise: strv(v.surprise) };
      case "commit":
        if (!isObj) return undefined;
        return { what: strv(v.what), why: strv(v.why) };
      case "progress":
        if (!isObj) return undefined;
        return { done: !!v.done, note: strv(v.note) };
      case "ikigai":
        if (!isObj) return undefined;
        return { like: strv(v.like), good: strv(v.good), need: strv(v.need), paid: strv(v.paid), center: strv(v.center) };
      case "priority":
        if (!isObj) return undefined;
        return { order: strArr(v.order), why: strv(v.why) };
      case "odyssey":
        if (!isObj) return undefined;
        plans = [];
        for (i = 0; i < 3; i++) {
          var p = (Array.isArray(v.plans) && v.plans[i] && typeof v.plans[i] === "object") ? v.plans[i] : {};
          plans.push({ t: strv(p.t), b: strv(p.b), e: isFinite(Number(p.e)) ? Number(p.e) : 0 });
        }
        return { plans: plans };
      case "fearSetting":
        if (!isObj) return undefined;
        return { define: strv(v.define), prevent: strv(v.prevent), repair: strv(v.repair) };
      default:
        return undefined;
    }
  }
  function sanitizeAnswers(ans) {
    ...
    COURSE.weeks.forEach(function (w) {
      w.steps.forEach(function (s) {
        if (!s.id || !(s.id in ans)) return;
        var v = cleanVal(s, ans[s.id]);
  ...
  function compChoices(s) {
    ...
    var chips = s.options.map(function (o) {
      ...
      return '<button class="chip' + on + '" onclick="toggleChoice(\'' + s.id + "','" + jstr(o) + "',this," + (s.multi ? "true" : "false") + ')">' + esc(o) + "</button>";
    }).join("");
  ...
  function compLog(s) {
    if (!Array.isArray(A[s.id])) A[s.id] = [];
    if (A[s.id].length < 7) {
      while (A[s.id].length < 7) { var o = {}; s.fields.forEach(function (f) { o[f.key] = ""; }); A[s.id].push(o); }
      save();
    }
    var data = A[s.id];
    var hint = s.hint ? '<p class="hint">' + esc(s.hint) + "</p>" : "";
    var blocks = data.map(function (row, i) {
      if (!row || typeof row !== "object") row = data[i] = {};
  ...
  function priorityData(id) {
    ...
    if (!priorityFilled(p)) {
      var src = (A.w3_journey && Array.isArray(A.w3_journey.v)) ? A.w3_journey.v.filter(function (x) { return x && x.trim(); }) : [];
      p.order = src.slice();
  ...
  function renderBook() {
    ...
    var w4 = getWeek("w4");
    var areaStep = w4 ? w4.steps.filter(function (s) { return s.type === "sliders"; })[0] : null;
    var areaItems = areaStep ? areaStep.items : [];
    var radarHtml = (areaItems.length && Object.keys(radarVals).length)
      ? '<svg class="radar" viewBox="0 0 220 220">' + radarInner(areaItems, radarVals) + "</svg>"
    ...
    var html =
      ...
      '<div class="bq"><h4>지금 내 삶의 영역</h4>' + radarHtml + "</div>" +
